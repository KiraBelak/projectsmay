...
public class LoadDatabase {
    ...
    CommandLineRunner initDatabase(ClientRepository repository, ProviderRepository providerRepository, PaymentRepository paymentRepository) {
        return args -> {
            ...
            //? Provider inserts
            log.info("Inserting into Providers " + providerRepository.save(new Provider("Las tortas del tio carlos","quimicos","Unos productos de limpieza")));
            log.info("Inserting into Providers " + providerRepository.save(new Provider("Bethortas vikingas","libros de tortas","Unas vikingas bien buenas")));
